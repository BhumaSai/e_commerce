...
auth.post('/register', async (req, res) => {
    try {
        const { Name, Mail, Mobile, Password, ConfirmPassword, State, District, Gender } = await req.body
...
auth.post('/login', async (req, res) => {
    try {
        ...
        res.cookie('Token', Token, { httpOnly: true, secure: false, expires: new Date(Date.now() + 86400000), sameSite: "none" })
